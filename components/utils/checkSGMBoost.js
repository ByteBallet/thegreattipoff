...
export default async function checkSGMBoost(multi, clientid, boostBal, reset, odds) {
    const res = await getMultiBoostDetails(multi, clientid, boostBal, reset);
    if (res && Object.keys(res).length > 0) {
        if ((res.boost.legstatuscode == "0" || res.boost.legstatuscode == 0) && res.boost.boostdiv) {
        ...
        multi[0].price = res.multierror ? res.multierror.multidiv : odds
