import { Typography, Badge, Box, Fade } from '@mui/material';
import { useEffect, useRef, useState, useContext } from 'react';
import ArrowDropDownIcon from '@mui/icons-material/ArrowDropDown';
import ArrowDropUpIcon from '@mui/icons-material/ArrowDropUp';
import { TipContext } from '../../../Context/Tip/TipProvider';
import { getBetID, getOddsValues } from '../../utils/RacingUtil';

function RaceOdds(props) {
    //check if odds null and populate it based on sprices obj
    let price = 0;
    let stype = '';
    let btype = '';
    let data = getOddsValues(
        props.raceField,
        props.win,
        props.productGroupType
    ).split('_');
    ...
    function checkTipSelected() {
        ...
        let tip_id =
            props.tipId.substr(0, idx + 1) +
            (props.productGroupType
                ? props.productGroupType.toLowerCase()
                : '');
        tip_id = props.win ? tip_id : tip_id.replace('win', 'place');
        let selected =
            tips.singles.filter((sTip) => getBetID(sTip) === tip_id).length > 0
                ? true
                : false;
        return selected;
    }
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Box
                ...
                sx={{
                    ...
                    border: 1,
                    borderColor:
                        props.raceResulted || props.raceStatus != 'open'
                            ? '#f5f5f5'
                            : 'transparent',
                    borderRadius: 2,
                    my: 1,
            ...
            >
                <Badge
                    badgeContent={
                        props.win &&
                        props.fav == 1 &&
                        props.productGroupType.toLowerCase() == 'fixed'
                            ? 'FAV'
                            : 0
                    }
                    color="primary"
